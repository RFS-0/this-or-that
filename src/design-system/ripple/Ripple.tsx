import { createSimpleEmitter, GenericEmit, GenericListen } from '@solid-primitives/event-bus'
...
type RippleEvent = { type: 'click' | 'contextmenu' | 'pointercancel' | 'pointerdown' | 'pointerenter' | 'pointerleave' | 'pointerup', pointerEvent?: Event | PointerEvent };

export function createRippleEventEmitter(): [listen: GenericListen<[RippleEvent]>, emit: GenericEmit<[RippleEvent]>, clear: VoidFunction] {
  return createSimpleEmitter<RippleEvent>()
}
...
export function createHandlers(emit: GenericEmit<[RippleEvent]>): RippleHandlers {
  return {
...
export type RippleProps = ParentProps & JSX.IntrinsicElements['div'] & {
  listen: GenericListen<[RippleEvent]>
  unbounded?: boolean
  disabled?: boolean
}
...
export const Ripple: ParentComponent<RippleProps> = (props) => {
  ...
  const getNormalizedPointerEventCoords: (pointerEvent: PointerEvent) => { x: number, y: number } = (pointerEvent) => {
  ...
  const endPress = () => {
    const pressAnimationPlayState = growAnimation?.currentTime ?? Infinity;
